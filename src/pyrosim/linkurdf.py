from src.pyrosim.originurdf      import ORIGIN_URDF

from src.pyrosim.geometryurdf    import GEOMETRY_URDF

from src.pyrosim.inertialurdf    import INERTIAL_URDF

from src.pyrosim.visualurdf      import VISUAL_URDF

from src.pyrosim.collisionurdf   import COLLISION_URDF

from src.pyrosim.commonFunctions import Save_Whitespace

class LINK_URDF:

    def __init__(self,name,pos,size,color):

        self.name = name

        self.depth = 1

        self.origin   = ORIGIN_URDF(pos)

        self.inertial  = INERTIAL_URDF(self.origin)

        self.geometry = GEOMETRY_URDF(size)

        self.visual    = VISUAL_URDF(self.origin , self.geometry,color=color)

        self.collision = COLLISION_URDF(self.origin , self.geometry)

    def Save(self,f):

        self.Save_Start_Tag(f)

        self.inertial.Save(f)

        self.visual.Save(f)

        self.collision.Save(f)

        self.Save_End_Tag(f)

# ------------------- Private methods -----------------

    def Save_End_Tag(self,f):

        Save_Whitespace(self.depth,f)

        f.write('</link>\n')

    def Save_Start_Tag(self,f):

        Save_Whitespace(self.depth,f)

        f.write('<link name="' + self.name + '">\n')
