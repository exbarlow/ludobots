...
class JOINT:
    def __init__(self,parent,face=None):
        ...
        if self.upstreamJoint == None:
            ...
            self.position = np.add(self.upstreamJoint.position,np.multiply(displacementVector,self.parent.dims/2))
    ...
    def getDisplacementVector(self,relative=True):
        ...
        return np.subtract(faceUnitVector,upstreamUnitVector)
